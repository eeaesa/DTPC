...
class UNet_DTPC(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_DTPC(params)
...
if __name__ == "__main__":
    ...
    model = UNet(class_num=4, in_chns=1).to('cuda') # 2.29 GMac 1.81 M
    print(model)
    int = torch.randn(2,3,224,224).cuda()
